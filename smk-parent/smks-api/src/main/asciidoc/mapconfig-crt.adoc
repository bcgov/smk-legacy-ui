== {POST} MapConfiguration - Create a new SMK Map Configuration (not for publishing)

Executing a POST at the MapConfiguration endpoint with a body containing the required Map Configuration json will create a new, unpublished Map Configuration that can be used for editing. All newly created documents will have their SMK Version set to '1'
