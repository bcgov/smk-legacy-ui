...
import ca.bc.gov.databc.smks.model.MapConfigInfo;
...
public class MapConfigController
{
	...
	public ResponseEntity<?> createMapConfig(@RequestBody MapConfiguration request)
	{
		...
		try
		{
			...
			if(request.getName() == null) throw new Exception("The LMF ID is null. This is a required field");
			if(request.getName().length() == 0) throw new Exception("The LMF ID is empty. Please fill in a valid field");
			...
			if(existingDocID != null) throw new Exception("The LMF ID is already in use. For creating new documents, ensure the ID is unique. If you are attempting to update an existing Map Configuration, please hit the following endpoint: {PUT}/MapConfigurations/{id}");
