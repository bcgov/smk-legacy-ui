== {GET} MapConfiguration/{id} - Retrieve the latest versions of a LMF Map Configurations (non-published only)

Executing a GET at the MapConfiguration endpoint with the LMF id will return the related Map Configuration document
